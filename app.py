...
app = Flask(__name__)  # Fix the incorrect `name`
...
def predict():
    ...
    try:
        ...
        custom_heatmap_path = os.path.join(app.config['UPLOAD_FOLDER'], f'customheatmap{filename}')
        resnet_heatmap_path = os.path.join(app.config['UPLOAD_FOLDER'], f'resnetheatmap{filename}')
        ...
        return jsonify({
            'custom_cnn': {
                ...
                'heatmap': url_for('static', filename=f'uploads/customheatmap{filename}')
            },
            'resnet101': {
                'probability': resnet_pred,
                'prediction': 'Pneumonia' if resnet_pred > 0.5 else 'Normal',
                'heatmap': url_for('static', filename=f'uploads/resnetheatmap{filename}')
            },
